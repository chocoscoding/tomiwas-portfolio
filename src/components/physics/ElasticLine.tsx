...
import { animate, motion, useAnimationFrame, useMotionValue, ValueAnimationTransition } from "framer-motion";
...
const ElasticLine: React.FC<ElasticLineProps> = ({
  ...
  const clampedReleaseThreshold = Math.min(releaseThreshold, isVertical ? dimensions.width / 2 : dimensions.height / 2);

  const { isGrabbed, controlPoint } = useElasticLineEvents(containerRef, isVertical, grabThreshold, clampedReleaseThreshold);
